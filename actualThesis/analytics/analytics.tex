...
In this chapter, we will attempt to derive analytic results about how material flows in the model. Initially,
...
Keeping the SPM on a ring, there is again a correspondence between it and the Misanthrope Process~\cite{evansWaclaw2014}. The Misanthrope Process is, like the SPM, defined by its rates. This time, however, there can be arbitrarily many particles
...
\end{equation}
where $f(m)$ is a weighting dependent on the occupation of a site,  we see that for the SPM 
\begin{equation}
 f(m) = 
 \begin{cases}
 \lambda, & n=0 \\
...
In terms of what a negative diffusion coefficient actually means, consider a constant solution $\rho(x, t) = \rho_0$. Insertion Eq.~\eqref{eq:contPDE} quickly confirms that this is indeed a solution. Now consider adding a small
