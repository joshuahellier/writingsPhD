...
eigenvalues must have negative real part. In physical terms, the system must have a single
...
\subsection{Another Worked Example: Open System}
...
We can also compute the eigenspectrum of the TRM for this small system;
this is displayed in Fig. \ref{fig:smallEigSpec}.
 ...
 The variation of the eigenspectrum of the TRM for our small open system with $\lambda$. Here we have plotted
 $-\Re{\lambda}$, as $\lambda$ is expected to be a complex number with negative real part.
 }
  \begin{center}
...
\begin{itemize}
 ...
 $L=5$, $b=1000$. Here we have plotted
 $-\Re{\lambda}$, as $\lambda$ is expected to be a complex number with negative real part.
 }
  \begin{center}
...
\end{itemize}
Of course, this is only the $32$ eigenvalues with real part closest to zero; there could be different behaviour
in the other ones. Before we perform a larger calculation however, let us turn our
...
\afterpage{
 ...
 The lower (negative real) TRM eigenspectrum, computed for a system with $\rho_0 = 0.6$, $\rho_L = 0.4$,
 ...
 at least in the digital copy.
  ...
  \begin{center}
 \includegraphics[width=0.9\textheight, angle=270]{TRM/images/repeatRepeatMultEig}
  \end{center}
\end{figure}
\clearpage
}
...
\begin{tabular}{r || r | r | r | r | r | r | r | r | r | r | r | r} \label{tab:bandThickness}
 ...
 \hline
 $\mathrm{N}^\mathrm{o}$. Slow Modes & 1 & 3 & 6 & 11 & 20 & 36 & 64 & 113 & 199 \\
\end{tabular}
\end{center}
\end{table}
...
it is rather obvious that this number scales exponentially with system size.
...
becomes large.
 ...
 $ \mathrm{N}^\mathrm{o} \mathrm{Slow Modes} = 2^{a(L+b)}$, with $a=0.84$, $b=-3.9$.
 }
  \begin{center}
...
Fig. \ref{fig:eigsVarBVarLUpper}.
 ...
 The upper (negative real) TRM eigenspectrum, computed for a system with $\rho_0 = 0.6$, $\rho_L = 0.4$,
 with all combinations of
 $L=\{5, 10\}$, $b=\{100, 1000\}$.
 }
  \begin{center}
 \includegraphics[width=0.9\textheight, angle=270]{TRM/images/topEigs}
  \end{center}
\end{figure}
...
\afterpage{
 ...
 The ``full'' (negative real) TRM eigenspectrum, computed for a system with $\rho_0 = 0.6$, $\rho_L = 0.4$,
...
\begin{itemize}
 ...
 Instead of the flow discontinuously switching to zero, the TRM results show a more gradual reduction of the current.
 ...
 current for
the $(0.6, 0.4)$ and $(0.9, 0.7)$-boundaried systems, observed around $\lambda=0.002$ and $\lambda=0.1$ respectively.
This could be due to a close-approach of the $0^\mathrm{th}$
and $1^\mathrm{st}$ eigenvalues,
causing the algorithm to pick the wrong one under certain circumstances. We have attempted to filter the results by
imposing the inequality $\| q_0 \| \le \lambda K$ for $K=10^{-9}$ with $q_0$ being the ``zero'' eigenvalue,
but some results may have slipped through this net. Likewise, this criterion may have been too stringent in some
cases, resulting in otherwise valid datapoints being removed (hence the gaps observed in the $(0.3, 0.1)$ data at higher
$\lambda$-values).
...
shouldn't trust those results. As $\lambda \rightarrow 0$, the space of vectors with eigenvalue zero
...
\begin{center}
 \begin{tabular}{c}
    \includegraphics[width=0.55\linewidth]{TRM/images/timeSeriesl0_1}  \\ 
    \includegraphics[width=0.55\linewidth]{TRM/images/timeSeriesl1_0}  \\
    \includegraphics[width=0.55\linewidth]{TRM/images/timeSeriesl2_0}  \\
    \end{tabular}
\end{center}
    \vspace{-2em}
\end{figure}
